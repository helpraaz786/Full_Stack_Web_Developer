...
var updateCollection = async (record) => {
    await client.connect();
    await client.db("GoTrendz").collection("userCredentials").updateOne({
        accountId: record.accountId}, {$set:{"isUserAlreadyLoggedin":true}});
}
        ...
        if (collectionResult.length == 1) {
            bcrypt.compare(req.body.accountPassword, collectionResult[0].accountPassword, function(err, result) {
                if (result) {
                    ...
                    if (collectionResult[0].isUserAlreadyLoggedin) {
                        resData.msg  = 'Invalid';
                        resData.info = 'User already loggedin in a different machine';
                    } else {
                        req.session.accountId = req.body.accountId;
                        updateCollection(collectionResult[0]);
                    }
